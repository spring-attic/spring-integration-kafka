...
public class KafkaProducerMessageHandler<K, V> extends AbstractMessageHandler {
	 ...
	 * The timeout is applied only in {@link #sync} mode.
	...
	/**
	 * Specify a SpEL expression to evaluate a timeout in milliseconds for
	 * how long {@link KafkaProducerMessageHandler}
	 * should wait wait for send operation results. Defaults to 10 seconds.
	 * The timeout is applied only in {@link #sync} mode.
	 * @param sendTimeoutExpression the {@link Expression} for timeout to wait for result fo send operation.
	 * @since 2.1.1
	 */
